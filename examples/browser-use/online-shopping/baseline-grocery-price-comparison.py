...
**Objective:**
Compare basket prices for 5 essential items across Migros, Coop, and Aldi online stores.
Find the CHEAPEST available option for each item and compare total basket costs.
...
### Step 2: Shop at Coop
- Visit https://www.coop.ch/en
- Search for the same 5 items
- Find the CHEAPEST option for each
- Add ONE of each to basket (don't checkout)
- Note: item name, brand, price, price per unit if shown
- Record total basket price

### Step 3: Shop at Aldi
- Visit https://www.aldi-suisse.ch/de/homepage.html
- Search for the same 5 items
- Find the CHEAPEST option for each
- Add ONE of each to basket (don't checkout)
- Note: item name, brand, price, price per unit if shown
...
Create a clear comparison table showing:
...
**PRICE COMPARISON WINNER:** [Store with lowest total]
**SAVINGS vs most expensive:** CHF [difference]
...
def parse_basket_data(output_text):
    ...
    # Look for store basket sections - the agent provides structured summaries
    store_patterns = {
        'MIGROS': r'(?i)\*?\*?MIGROS\s+BASKET:\*?\*?\s*(.*?)(?=\*?\*?(?:COOP|ALDI)\s+BASKET|---|\*?\*?TOTAL|$)',
        'COOP': r'(?i)\*?\*?COOP\s+BASKET:\*?\*?\s*(.*?)(?=\*?\*?(?:MIGROS|ALDI)\s+BASKET|---|\*?\*?TOTAL|$)',
        'ALDI': r'(?i)\*?\*?ALDI\s+BASKET:\*?\*?\s*(.*?)(?=\*?\*?(?:MIGROS|COOP)\s+BASKET|---|\*?\*?TOTAL|$)'
    }
    ...
    for store, pattern in store_patterns.items():
        ...
        if match:
            ...
            # Look for total in this store's section or in the general output
            total_pattern = r'(?i)\*?\*?TOTAL:\s*CHF\s*(\d+(?:\.\d{2})?)\*?\*?'
            # First try to find total in this specific basket section
            total_match = re.search(total_pattern, basket_section)
            if not total_match:
                # Then try to find it in the whole output near this store
                store_section_pattern = f'(?i){store}.*?CHF\\s*(\\d+(?:\\.\\d{{2}})?)\\s*(?=\\n|$|\\.)'
                total_match = re.search(store_section_pattern, output_text)

            if total_match:
                total_value = float(total_match.group(1))
                total = f"TOTAL: CHF {total_value}"

            if items:  # Only add store data if we found items
                stores_data[store] = {
                    'items': items,
                    'total': total,
...
def print_results_summary(output_text):
    ...
    print(f"🏪 Stores: Migros, Coop, Aldi")
    ...
    if stores_data:
        ...
        # Note: Price comparison removed per user request
    else:
...
async def run_grocery_comparison():
    ...
    print("🛒 Starting grocery price comparison...")
    print("Comparing 5 essential items across Migros, Coop, and Aldi")
    print("-" * 50)
...
async def main():
    # Run grocery comparison and collect metrics
    result = await run_grocery_comparison()
